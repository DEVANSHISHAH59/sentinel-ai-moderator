...
def run_pipeline(text: str, locale: str = "en") -> Dict[str, Any]:
    ...
    Returns plain dicts so it deploys easily on Streamlit Cloud.
    """
    ...
    if "urgent" in t or "act now" in t or "limited time" in t or "final notice" in t:
        scam_score = max(scam_score, 0.40)
        scam_reasons.append("urgency_pressure")
    if "crypto" in t or "bitcoin" in t or "wallet address" in t:
        scam_score = max(scam_score, 0.55)
        scam_reasons.append("crypto_payment_request")

    if scam_score > 0:
        findings.append({"category": "scam_fraud", "severity": scam_score, "reasons": scam_reasons})
    ...
    if sex_score > 0:
        findings.append({"category": "sexual_content", "severity": sex_score, "reasons": sex_reasons})
    ...
    if "older men" in t or "older women" in t or "age gap" in t or "meet privately" in t:
        child_score = max(child_score, 0.60)
        child_reasons.append("adult_minor_context")

    if child_score > 0:
        findings.append({"category": "child_safety", "severity": child_score, "reasons": child_reasons})
    ...
    # Show/hide decision: block if very high severity or child-safety is high
    safe_to_show = True
    if overall >= 0.70:
        safe_to_show = False
    ...
    return {
        ...
        "findings": findings,
        "debug": {"locale": locale, "length": len(text or "")},
    }
